...
<form action="./fun/addCard.php" method="post" target="resultbox">
    ...
    <div class="inputbox"><span>允许报名课程：</span><br>
        <?php
        require_once("../config/database.php");
        $courses = mysqli_query($db, "SELECT cid, competition_name FROM course");
        while ($row = mysqli_fetch_assoc($courses)) {
            echo '<label><input type="checkbox" name="allowed_courses[]" value="' . $row["cid"] . '"> ' . htmlspecialchars($row["competition_name"]) . '</label><br>';
        }
        mysqli_close($db);
        ?>
    </div>
